...
Ko=15;  % constant for kic mode 1, mpa/l
sigmac=20; % in Kic
Ko2=400;  % constant for kic mode 2, mpa/l
sigmac2=280; % in Kic
co=4; % cohesion 
mu=0.1; % friction angel
cc1=400;
cc2=15;
gamap=0;
 cstep=500;
...
sigmamt{1}=[0 0 0; 0 0 0; 0 0 -37.0]; % global total stress
sigmamt{2}=[0 0 0; 0 0 0; 0 0 -38.5]; % global total stress
...
sigmam=[0 0 0; 0 0 0; 0 0 0]; % current global stress
...
oam=am(i);
if fdm(i)>0
am(i)=newam(sigmacn(i),am(i),Ko,sigmac,tol);
end
...
aepsilon=zeros(tstep,1);
lepsilon=zeros(tstep,1);
astress=zeros(tstep,1);
damage1=zeros(tstep,1);
damage2=zeros(tstep,1);
damage3=zeros(tstep,1);

damagem1=zeros(tstep,1);
damagem2=zeros(tstep,1);
damagem3=zeros(tstep,1);
damagew1=zeros(tstep,1);
damagew2=zeros(tstep,1);
damagew3=zeros(tstep,1);

for i=1:tstep
    aepsilon(i)=tstrain{i}(3,3);
    lepsilon(i)=tstrain{i}(1,1);    
    astress(i)=tstress{i}(3,3);    
    damage1(i)=tdamage{i}(1,1);  
    damage2(i)=tdamage{i}(2,2); 
    damage3(i)=tdamage{i}(3,3); 
    damagem1(i)=tdamagem{i}(1,1);  
    damagem2(i)=tdamagem{i}(2,2); 
    damagem3(i)=tdamagem{i}(3,3); 
    damagew1(i)=tdamagew{i}(1,1);  
    damagew2(i)=tdamagew{i}(2,2); 
    damagew3(i)=tdamagew{i}(3,3); 
end
...
cruves{1}=plot(-aepsilon,-astress,'-k','LineWidth',1);
hold on
cruves{2}=plot(expstraina,expstressd,'-r','LineWidth',1);
hold on
axis([ 0,0.05,0, 50])
set(gcf, 'position', [200 200 400 300]);
set(gca,'FontSize',14)
xlabel('Axial strain','fontsize',14)
ylabel('Deviatoric stress (MPa)','fontsize',14)
legend([cruves{1},cruves{2}],'Wing cracks model','Experiments','Location','southeast')

figure('Name','stress-strain','NumberTitle','off')
cruves{1}=plot(-lepsilon,-astress,'-k','LineWidth',1);
hold on
cruves{2}=plot(expstrainl,expstressd,'-r','LineWidth',1);
hold on
axis([-0.025,0,0, 50])
set(gcf, 'position', [200 200 400 300]);
set(gca,'FontSize',14)
...
% fisrt cycle
figure('Name','elastic stress-strain','NumberTitle','off')
cruves{1}=plot(expstraina(716:3350),expstressd(716:3350),'-k','LineWidth',1);
hold on
cruves{2}=plot(expstraina(3351:end),expstressd(3351:end),'--k','LineWidth',1);
hold on
plot(expstrainl(716:3350),expstressd(716:3350),'-k','LineWidth',1);
plot(expstrainl(3351:end),expstressd(3351:end),'--k','LineWidth',1);
axis([-0.0002,0.0005,0, 10])
set(gcf, 'position', [200 200 420 350]);
set(gca,'FontSize',14)
xlabel('Lateral strain  <--->  Axial strain                                      ','fontsize',14)
ylabel('Differential stress (MPa)','fontsize',14)
legend([cruves{1},cruves{2}],'The first cycle','Reloading','Location','southeast')
...
cruves{2}=plot(expstraina,expstressd,'-r','LineWidth',1);
hold on
cruves{3}=plot(-lepsilon,-astress,'--k','LineWidth',1);
hold on
cruves{4}=plot(expstrainl,expstressd,'--r','LineWidth',1);
hold on
axis([ -0.05,0.05,0, 50])
...
xlabel('Lateral strain  <--->  Axial strain    ','fontsize',14)
ylabel('Deviatoric stress (MPa)','fontsize',14)
legend([cruves{1},cruves{2},cruves{3},cruves{4}],'Wing cracks model (Axial)'...
    ,'Experiments (Axial)','Wing cracks model (Lateral)','Experiments (Lateral)','Location','southwest')
% 
figure('Name','stress-damage','NumberTitle','off')
cruves{1}=plot(-astress,damage1,'-k','LineWidth',1);
hold on
cruves{3}=plot(-astress,damage3,'-r','LineWidth',1);
hold on
axis([ 0,60,0, 0.7])
set(gcf, 'position', [200 200 440 300]);
set(gca,'FontSize',14)
xlabel('Deviatoric stress (MPa)','fontsize',14)
ylabel('\Omega','fontsize',14)
legend([cruves{1},cruves{3}],'\Omega_1, \Omega_2 ','\Omega_3','Location','southeast')


figure('Name','stress-damage','NumberTitle','off')
cruves{1}=plot(-astress,damagem1,'-k','LineWidth',1);
hold on
cruves{3}=plot(-astress,damagem3,'-r','LineWidth',1);
hold on
set(gcf, 'position', [200 200 400 300]);
set(gca,'FontSize',14)
axis([ 0,50,0, 0.4])
xlabel('Deviatoric stress (MPa)','fontsize',14)
ylabel('\Omega_m','fontsize',14)
legend([cruves{1},cruves{3}],'\Omega_{m1}, \Omega_{m2} ','\Omega_{m3}','Location','southeast')

figure('Name','stress-damage','NumberTitle','off')
cruves{1}=plot(-astress,damagew1,'-k','LineWidth',1);
hold on
cruves{3}=plot(-astress,damagew3,'-r','LineWidth',1);
hold on
set(gcf, 'position', [200 200 400 300]);
set(gca,'FontSize',14)
axis([ 0,50,0, 0.4])
xlabel('Deviatoric stress (MPa)','fontsize',14)
ylabel('\Omega_w','fontsize',14)
legend([cruves{1},cruves{3}],'\Omega_{w1}, \Omega_{w2} ','\Omega_{w3}','Location','southeast')
...
cruves{1}=plot(-astress,damage1,'-k','LineWidth',1);
hold on
cruves{2}=plot(-astress,damage2,'-r','LineWidth',1);
hold on
cruves{3}=plot(-astress,damage3,'-b','LineWidth',1);
hold on
set(gcf, 'position', [200 200 400 300]);
set(gca,'FontSize',14)
xlabel('Deviatoric stress (MPa)','fontsize',14)
ylabel('Damage','fontsize',14)
legend([cruves{1},cruves{2},cruves{3}],'Damage 1','Damage 2','Damage 3','Location','best')


figure('Name','stress-damage','NumberTitle','off')
cruves{1}=plot(astress,damagem1,'-k','LineWidth',1);
hold on
cruves{2}=plot(astress,damagem2,'-r','LineWidth',1);
hold on
cruves{3}=plot(astress,damagem3,'-b','LineWidth',1);
hold on
set(gcf, 'position', [200 200 400 300]);
set(gca,'FontSize',14)
xlabel('Deviatoric stress (MPa)','fontsize',14)
...
cruves{1}=plot(astress,damagew1,'-k','LineWidth',1);
hold on
cruves{2}=plot(astress,damagew2,'-r','LineWidth',1);
hold on
cruves{3}=plot(astress,damagew3,'-b','LineWidth',1);
hold on
set(gcf, 'position', [200 200 400 300]);
set(gca,'FontSize',14)
xlabel('Deviatoric stress (MPa)','fontsize',14)
