...
sigmamt{1}=[0 0 0; 0 0 0; 0 0 -38.38]; % global total stress
if senn==2
    sigmamt{1}=[0 0 0; 0 0 0; 0 0 -42.13]; % global total stress
end
if senn==3
        sigmamt{1}=[0 0 0; 0 0 0; 0 0 -45.21]; % global total stress
end
...
sigmam=[confine(senn) 0 0; 0 confine(senn) 0; 0 0 confine(senn)]; % current global stress
...
for i=1:tstep
    aepsilon(i,senn)=tstrain{i}(3,3)-tstrain{1}(3,3);
    lepsilon(i,senn)=tstrain{i}(1,1)-tstrain{1}(1,1);    
    astress(i,senn)=tstress{i}(3,3)-confine(senn);    
...
hold on
axis([ -0.012,0.012,0, 50])
set(gcf, 'position', [200 200 700 350]);
set(gca,'FontSize',14)
xlabel('Lateral strain  <--->  Axial strain    ','fontsize',14)
ylabel('Differential stress (MPa)','fontsize',14)
legend([cruves{1},cruves{2},cruves{3},cruves{4},cruves{5},cruves{6}],'p=0 MPa (Axial)',...
    'p=5 MPa (Axial)','p=10 MPa (Axial)','p=0 MPa (Lateral)',...
    'p=5 MPa (Lateral)','p=10 MPa (Lateral)','Location','southwest')
...
ylabel('Differential stress (MPa)','fontsize',14)
legend([cruves{1},cruves{2},cruves{3}],'p=0 MPa','p=5 MPa','p=10 MPa','Location','southeast')
...
legend([cruves{1},cruves{2},cruves{3}],'p=0 MPa','p=5 MPa','p=10 MPa','Location','northeast')
