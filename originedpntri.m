% variable initiation
clear;
Em=21000; %young modulus mpa
nu=0.32; % poisson ratio

Ec=21000; %young modulus mpa of crack
...
sigmamt{1}=[0 0 0; 0 0 0; 0 0 -37]; % global total stress
sigmamt{2}=[0 0 0; 0 0 0; 0 0 -38.5]; % global total stress
sigmamt{3}=[0 0 0; 0 0 0; 0 0 -39.18]; % global total stress
sigmamt{4}=[0 0 0; 0 0 0; 0 0 -39.48]; % global total stress
sigmamt{5}=[0 0 0; 0 0 0; 0 0 -39.611]; % global total stress
sigmamt{6}=[0 0 0; 0 0 0; 0 0 -39.745]; % global total stress
sigmamt{7}=[0 0 0; 0 0 0; 0 0 -39.808]; % global total stress
...
if abs(tempstrainmc(1,1))>1
   tempstrainmc=zeros(3,3);
end 
...
hold on
axis([ 0,0.05,0, 50])
set(gcf, 'position', [200 200 430 300]);
set(gca,'FontSize',14)
xlabel('Axial strain','fontsize',14)
ylabel('Differential stress (MPa)','fontsize',14)
legend([cruves{1},cruves{2}],'DWCPD model','Experiments','Location','northeast')
...
ylabel('Differential stress (MPa)','fontsize',14)
legend([cruves{1},cruves{2},cruves{3},cruves{4}],'Model (Axial)'...
    ,'Experiments (Axial)','Model (Lateral)','Experiments (Lateral)','Location','southwest')
% 
figure('Name','stress-strain','NumberTitle','off')

cruves{2}=plot(expstraina,expstressd,'-k','LineWidth',1.3);
hold on

cruves{4}=plot(expstrainl,expstressd,'-k','LineWidth',1.3);
hold on
axis([ -0.03,0.05,0, 45])
set(gcf, 'position', [200 200 700 350]);
set(gca,'FontSize',14)
xlabel('Lateral strain  <--->  Axial strain                                       ','fontsize',14)
ylabel('Differential stress (MPa)','fontsize',14)
%legend([cruves{2},cruves{4}],'Experiments (Axial)','Experiments (Lateral)','Location','northwest')
% 
...
legend([cruves{1},cruves{3}],'\Omega_{w1}, \Omega_{w2} ','\Omega_{w3}','Location','northeast')
